...
async function searchUser(){
  ...
  try {
    ...
    // https://api.github.com/users/olekwa
...
async function fetchRepositories(reposUrl) {
  reposContainer.innerHTML = '<div class="loading-repos">Loading respositories...</div>'
...
function displayRepos(repos){
  if(repos.length === 0) {
    reposContainer.innerHTML = '<div class="no-repos">No respositories found</div>';
    return;
  }
...
function displayUserData(user){
  ...
  if (user.company) {
    ...
    companyElement.textContent = 'Not specifed';
  }
  ...
  if (user.twitter_username) {
    ...
    twitterElement.textContent = 'No X';
    twitterElement.href = '#';
  }
